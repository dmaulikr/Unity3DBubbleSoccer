...
public class PhotonRigidbodyView : MonoBehaviour, IPunObservable
{
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.isWriting == true)
        {
			...
			if (this.m_SynchronizeMass == true) {
				stream.SendNext (this.m_Body.mass);
			}

			if (this.m_SynchronizePosition == true) {
        ...
        else
        {
			...
			if (this.m_SynchronizeMass == true)
			{
				this.m_Body.mass = (float)stream.ReceiveNext();
			}

			if (this.m_SynchronizePosition == true) {
				this.m_Body.position = (Vector3)stream.ReceiveNext ();
			}
        }
    }
}